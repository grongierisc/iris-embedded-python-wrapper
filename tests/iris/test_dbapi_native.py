...
def test_dbapi_native_connect_errors_use_facade_exception_hierarchy(monkeypatch):
    iris.runtime.bind_backends(native_dbapi_module=None)
    ...
    class FakeNativeDBAPI:
        ...
        @staticmethod
        def connect(*args, **kwargs):
            raise FakeNativeDBAPI.OperationalError("access denied")
    ...
    with pytest.raises(iris.dbapi.OperationalError) as exc_info:
        iris.dbapi.connect(mode="native", hostname="localhost", port=1972)

    assert str(exc_info.value) == "access denied"
    assert isinstance(exc_info.value.__cause__, FakeNativeDBAPI.OperationalError)


def test_dbapi_native_cursor_errors_use_facade_exception_hierarchy(monkeypatch):
    iris.runtime.bind_backends(native_dbapi_module=None)
    ...
    class FakeNativeCursor:
        ...
        def execute(self, operation, params=None):
            raise FakeNativeDBAPI.IntegrityError("failed unique check")
    ...
    with pytest.raises(iris.dbapi.IntegrityError) as exc_info:
        cur.execute("insert into Demo.T (id) values (?)", (1,))

    assert str(exc_info.value) == "failed unique check"
    assert isinstance(exc_info.value, iris.dbapi.DatabaseError)
    assert isinstance(exc_info.value.__cause__, FakeNativeDBAPI.IntegrityError)
